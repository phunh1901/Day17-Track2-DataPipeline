...
{% macro priority_reject_reason(col) %}
    case
        ...
        when try_cast({{ col }} as integer) is not null and try_cast({{ col }} as integer) not between 1 and 4 then 'Số nguyên ngoài khoảng 1..4'
        else 'Chuỗi định dạng không hợp lệ'
    end
{% endmacro %}
