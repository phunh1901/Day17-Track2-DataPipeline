...
def consume(
    ...
    with LogConsumer(topic, offset_file) as consumer:
        ...
        while True:
            ...
            # Ghi dữ liệu trước (at-least-once + idempotent write), commit offset sau
            write_batch(con, batch)           # ghi dữ liệu
            maybe_crash(batch_no, crash_at)   # sự cố có thể xảy ra tại đây
            consumer.commit()                 # ghi nhận offset sau khi ghi thành công
